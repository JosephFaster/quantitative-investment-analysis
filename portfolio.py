...
class manager:
    ...
    def compute_portfolio(self, portfolio_type=None, target_return=None):
        ...
        # Define the maximum weight per asset for Markowitz optimization
        max_weight = 0.49
        bounds = [(0, max_weight) for _ in range(len(self.rics))]
        ...
        if portfolio_type == 'min-variance-l1':
            ...
            optimal_result = op.minimize(fun=portfolio_variance, x0=x0,
                                        args=(self.mtx_var_covar),
                                        constraints=(l1_norm),
                                        bounds=bounds)  # Apply bounds to long-only
            weights = np.array(optimal_result.x)
            ...
            optimal_result = op.minimize(fun=portfolio_variance, x0=x0,
                                        args=(self.mtx_var_covar),
                                        constraints=(l1_norm + markowitz),
                                        bounds=bounds)  # Apply bounds to Markowitz
            weights = np.array(optimal_result.x)
        ...
        # Normalize weights with L1 norm
        weights /= sum(abs(weights))
